...
static is_string_too_long = false;
...
<STRING>\"  {
  ...
  cool_yylval.symbol=stringtab.add_string(yytext);// TODO: better approach to handle too long string.
  return STR_CONST;
}

<STRING>. {
  if(!is_string_too_long && string_length==MAX_STR_CONST){
    is_string_too_long = true;
    strcpy(string_buf_ptr,"String constant too long");
    cool_yylval.error_msg=string_buf_ptr;
    return ERROR;
  }else if(is_string_too_long){
    string_length++;
    yymore();
  }else_exp}
}
