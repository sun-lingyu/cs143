...
class EvalCommand inherits Object{
   evaluate(head: StackCommand): Object{
      if (not isvoid(head.getNext())) then
      head.setNext(.eval())
   };
};
...
class DisplayCommand inherits IO{
   display(head: StackCommand): Object{
      let p: StackCommand <- head.getNext() in{
         ...
         {
            case p of
               intCommand: IntCommand => {out_int(intCommand.getVal()); out_string("\n");};
   ...
   main() : Object {
      {
         ...
         {
            if (input = "+") then
               let plusCommand: PlusCommand <- new PlusCommand in{
                  plusCommand.setNext(head.getNext());
                  head.setNext(plusCommand);
               }
            else 
               if (input = "s") then
                  let swapCommand: SwapCommand <- new SwapCommand in{
                     swapCommand.setNext(head.getNext());
                     head.setNext(swapCommand);
                  }
               else 
                  if (input = "e") then
                     ...
                     if (input = "d") then
                        ...
                        let intCommand: IntCommand <- new IntCommand in{
                           ...
                           intCommand.setNext(head.getNext());
